...
# -------------------------------------------------------------------------------
# Before we go searching, check whether boost-cmake is available, unless the user
# specifically asked NOT to search for boost-cmake.
#
# If Boost_DIR is set, this behaves as any find_package call would. If not, it looks at
# BOOST_ROOT and BOOSTROOT to find Boost.
#
if(NOT Boost_NO_BOOST_CMAKE)
    # If Boost_DIR is not set, look for BOOSTROOT and BOOST_ROOT as alternatives, since
    # these are more conventional for Boost.
    ...
    # Do the same find_package call but look specifically for the CMake version. Note that
    # args are passed in the Boost_FIND_xxxxx variables, so there is no need to delegate
    # them to this find_package call.
    find_package(Boost QUIET NO_MODULE)
    mark_as_advanced(Boost_DIR)

    # If we found boost-cmake, then we're done.  Print out what we found. Otherwise let
    # the rest of the module try to find it.
    if(Boost_FOUND)
...
#
# ########################################################################################
# This macro was taken directly from the FindQt4.cmake file that is included with the
# CMake distribution. This is NOT my work. All work was done by the original authors of
# the FindQt4.cmake file. Only minor modifications were made to remove references to Qt
# and make this file more generally applicable And ELSE/ENDIF pairs were removed for
# readability.
# ########################################################################################
...
macro(_Boost_ADJUST_LIB_VARS basename)
    if(Boost_INCLUDE_DIR)
        if(Boost_${basename}_LIBRARY_DEBUG AND Boost_${basename}_LIBRARY_RELEASE)
            ...
            if(_isMultiConfig OR CMAKE_BUILD_TYPE)
                set(Boost_${basename}_LIBRARY
                    optimized ${Boost_${basename}_LIBRARY_RELEASE} debug
                    ${Boost_${basename}_LIBRARY_DEBUG})
            else()
                # For single-config generators where CMAKE_BUILD_TYPE has no value, just
                # use the release libraries
                set(Boost_${basename}_LIBRARY ${Boost_${basename}_LIBRARY_RELEASE})
            endif()
            # FIXME: This probably should be set for both cases
            set(Boost_${basename}_LIBRARIES
                optimized ${Boost_${basename}_LIBRARY_RELEASE} debug
                ${Boost_${basename}_LIBRARY_DEBUG})
        endif()
        ...
        # if only the debug version was found, set the release variable also to the debug
        # version
        ...
        if("${Boost_${basename}_LIBRARY_RELEASE}" STREQUAL
           "${Boost_${basename}_LIBRARY_DEBUG}")
            set(Boost_${basename}_LIBRARY ${Boost_${basename}_LIBRARY_RELEASE})
            set(Boost_${basename}_LIBRARIES ${Boost_${basename}_LIBRARY_RELEASE})
        endif()
    ...
    mark_as_advanced(Boost_${basename}_LIBRARY_RELEASE Boost_${basename}_LIBRARY_DEBUG)
endmacro()

# Detect changes in used variables. Compares the current variable value with the last one.
# In short form: v != v_LAST                      -> CHANGED = 1 v is defined, v_LAST not
# -> CHANGED = 1 v is not defined, but v_LAST is  -> CHANGED = 1 otherwise -> CHANGED = 0
...
#
# Find the given library (var). Use 'build_type' to support different lib paths for
...
macro(_Boost_FIND_LIBRARY var build_type)
    ...
    if(${var})
        ...
        if(NOT Boost_LIBRARY_DIR_${build_type})
            ...
            set(Boost_LIBRARY_DIR_${build_type}
                "${_dir}"
                CACHE PATH "Boost library directory ${build_type}" FORCE)
        endif()
...
function(_Boost_COMPILER_DUMPVERSION _OUTPUT_VERSION _OUTPUT_VERSION_MAJOR
    ...
    set(${_OUTPUT_VERSION}
        ...
        PARENT_SCOPE)
    set(${_OUTPUT_VERSION_MAJOR}
        ${_boost_COMPILER_VERSION_MAJOR}
        PARENT_SCOPE)
    set(${_OUTPUT_VERSION_MINOR}
        ${_boost_COMPILER_VERSION_MINOR}
        PARENT_SCOPE)
endfunction()

#
# Take a list of libraries with "thread" in it and prepend duplicates with
# "thread_${Boost_THREADAPI}" at the front of the list
#
function(_Boost_PREPEND_LIST_WITH_THREADAPI _output)
    set(_orig_libnames ${ARGN})
    string(REPLACE "thread" "thread_${Boost_THREADAPI}" _threadapi_libnames
                   "${_orig_libnames}")
    set(${_output}
        ${_threadapi_libnames} ${_orig_libnames}
        PARENT_SCOPE)
endfunction()
...
function(_Boost_CHECK_SPELLING _var)
    if(${_var})
        ...
        message(
            FATAL_ERROR
                "ERROR: ${_var} is not the correct spelling.  The proper spelling is ${_var_UC}."
            )
    endif()
endfunction()

# Guesses Boost's compiler prefix used in built library names Returns the guess by setting
# the variable pointed to by _ret
function(_Boost_GUESS_COMPILER_PREFIX _ret)
    ...
    elseif(UNIX)
        ...
        if(NOT Boost_VERSION VERSION_LESS 106900)
            ...
            # those compilers, patch is exclude from compiler tag in Boost 1.69+ library
            # naming.
            ...
            elseif(CMAKE_CXX_COMPILER_ID STREQUAL "Clang"
                   AND _boost_COMPILER_VERSION_MAJOR VERSION_GREATER 3)
                set(_boost_COMPILER_VERSION "${_boost_COMPILER_VERSION_MAJOR}")
            endif()
        endif()
                ...
                if(APPLE)
                    if(Boost_MINOR_VERSION)
                        if(${Boost_MINOR_VERSION} GREATER 35)
                            # In Boost 1.36.0 and newer, the mangled compiler name used on
                            # macOS/Darwin is "xgcc".
...
#
# Get component dependencies.  Requires the dependencies to have been defined for the
# Boost release version.
#
# component - the component to check _ret - list of library dependencies
#
function(_Boost_COMPONENT_DEPENDENCIES component _ret)
    ...
    #
    # The output may be added in a new block below.  If it's the same as the previous
    # release, simply update the version range of the block for the previous release. Also
    # check if any new components have been added, and add any new components to
    # _Boost_COMPONENT_HEADERS.
    #
    # This information was originally generated by running BoostScanDeps.cmake against
    # every boost release to date supported by FindBoost:
    #
    # % for version in /path/to/boost/sources/* do cmake -DBOOST_DIR=$version -P
    # Utilities/Scripts/BoostScanDeps.cmake done
    #
    # The output was then updated by search and replace with these regexes:
    #
    # * Strip message(STATUS) prefix dashes s;^-- ;;
    # * Indent s;^set(;    set(;;
    # * Add conditionals s;Scanning /path/to/boost/sources/boost_\(.*\)_\(.*\)_\(.*);
    #   elseif(NOT Boost_VERSION VERSION_LESS \10\20\3 AND Boost_VERSION VERSION_LESS
    #   xxxx);
    #
    # This results in the logic seen below, but will require the xxxx replacing with the
    # following Boost release version (or the next minor version to be released, e.g. 1.59
    # was the latest at the time of writing, making 1.60 the next, so 106000 is the needed
    # version number).  Identical consecutive releases were then merged together by
    # updating the end range of the first block and removing the following redundant
    # blocks.
    #
    # Running the script against all historical releases should be required only if the
    # BoostScanDeps.cmake script logic is changed. The addition of a new release should
    # only require it to be run against the new release.
    ...
    if(Boost_VERSION AND Boost_VERSION VERSION_LESS 103300)
        message(
            WARNING
                "Imported targets and dependency information not available for Boost version ${Boost_VERSION} (all versions older than 1.33)"
            )
...
#
# Get component headers.  This is the primary header (or headers) for a given component,
# and is used to check that the headers are present as well as the library itself as an
# extra sanity check of the build environment.
#
# component - the component to check _hdrs
#
function(_Boost_COMPONENT_HEADERS component _hdrs)
    ...
    # Note: new boost components will require adding here.  The header must be present in
    # all versions of Boost providing a library.
    ...
    string(TOUPPER ${component} uppercomponent)
    set(${_hdrs}
        ${_Boost_${uppercomponent}_HEADERS}
        PARENT_SCOPE)
...
#
# Sets _Boost_${COMPONENT}_DEPENDENCIES for each required component, plus
# _Boost_IMPORTED_TARGETS (TRUE if imported targets should be defined; FALSE if dependency
# information is unavailable).
#
# componentvar - the component list variable name extravar - the indirect dependency list
# variable name
#
function(_Boost_MISSING_DEPENDENCIES componentvar extravar)
    ...
    # _boost_processed_components - components already processed (or currently being
    # processed) _boost_new_components - new components discovered for future processing
    #
    ...
    while(_boost_unprocessed_components)
        ...
        foreach(component ${_boost_unprocessed_components})
            ...
            foreach(componentdep ${_Boost_${uppercomponent}_DEPENDENCIES})
                if(NOT ("${componentdep}" IN_LIST _boost_processed_components
                        OR "${componentdep}" IN_LIST _boost_new_components))
    ...
    set(${componentvar}
        ${_boost_processed_components}
        PARENT_SCOPE)
    set(${extravar}
        ${_boost_extra_components}
        PARENT_SCOPE)
endfunction()

#
# Some boost libraries may require particular set of compler features. The very first one
# was `boost::fiber` introduced in Boost 1.62. One can check required compiler features of
# it in `${Boost_ROOT}/libs/fiber/build/Jamfile.v2`.
#
function(_Boost_COMPILER_FEATURES component _ret)
    ...
    string(TOUPPER ${component} uppercomponent)
    set(${_ret}
        ${_Boost_${uppercomponent}_COMPILER_FEATURES}
        PARENT_SCOPE)
endfunction()

#
# Update library search directory hint variable with paths used by prebuilt boost
# binaries.
#
# Prebuilt windows binaries (https://sourceforge.net/projects/boost/files/boost-binaries/)
# have library directories named using MSVC compiler version and architecture. This
# function would append corresponding directories if MSVC is a current compiler, so having
# `BOOST_ROOT` would be enough to specify to find everything.
#
function(_Boost_UPDATE_WINDOWS_LIBRARY_SEARCH_DIRS_WITH_PREBUILT_PATHS componentlibvar
         basedir)
...
# If the user sets Boost_LIBRARY_DIR, use it as the default for both configurations.
...
if(Boost_FIND_VERSION AND NOT Boost_FIND_VERSION_MINOR)
    message(
        SEND_ERROR
            "When requesting a specific version of Boost, you must provide at least the major and minor version numbers, e.g., 1.34"
        )
endif()

if(Boost_FIND_VERSION_EXACT)
    # The version may appear in a directory with or without the patch level, even when the
    # patch level is non-zero.
    set(_boost_TEST_VERSIONS
        "${Boost_FIND_VERSION_MAJOR}.${Boost_FIND_VERSION_MINOR}.${Boost_FIND_VERSION_PATCH}"
        "${Boost_FIND_VERSION_MAJOR}.${Boost_FIND_VERSION_MINOR}")
else()
    # The user has not requested an exact version.  Among known versions, find those that
    # are acceptable to the user request.
    #
    # Note: When adding a new Boost release, also update the dependency information in
    # _Boost_COMPONENT_DEPENDENCIES and _Boost_COMPONENT_HEADERS.  See the instructions at
    # the top of _Boost_COMPONENT_DEPENDENCIES.
    ...
    if(Boost_FIND_VERSION)
        ...
        foreach(version ${_Boost_KNOWN_VERSIONS})
            ...
            elseif("${version}.99" VERSION_EQUAL "${_Boost_FIND_VERSION_SHORT}.99")
                # This version is a short-form for the requested version with the patch
                # level dropped.
                list(APPEND _boost_TEST_VERSIONS "${version}")
            endif()
        endforeach()
...
# The reason that we failed to find Boost. This will be set to a user-friendly message
# when we fail to find some necessary piece of Boost.
...
# Supply Boost_LIB_DIAGNOSTIC_DEFINITIONS as a convenience target. It will only contain
# any interface definitions on WIN32, but is created on all platforms to keep end user
# code free from platform dependent code.  Also provide convenience targets to disable
# autolinking and enable dynamic linking.
...
if(WIN32)
    # In windows, automatic linking is performed, so you do not have to specify the
    # libraries.  If you are linking to a dynamic runtime, then you can choose to link to
    # either a static or a dynamic Boost library, the default is to do a static link.  You
    # can alter this for a specific library "whatever" by defining BOOST_WHATEVER_DYN_LINK
    # to force Boost library "whatever" to be linked dynamically.  Alternatively you can
    # force all Boost libraries to dynamic link by defining BOOST_ALL_DYN_LINK.
    ...
    # BOOST_LIB_DIAGNOSTIC will cause the auto-linking code to emit a #pragma message each
    # time a library is selected for linking.
    set(Boost_LIB_DIAGNOSTIC_DEFINITIONS "-DBOOST_LIB_DIAGNOSTIC")
    set_target_properties(Boost::diagnostic_definitions
                          PROPERTIES INTERFACE_COMPILE_DEFINITIONS "BOOST_LIB_DIAGNOSTIC")
    set_target_properties(Boost::disable_autolinking
                          PROPERTIES INTERFACE_COMPILE_DEFINITIONS "BOOST_ALL_NO_LIB")
endif()
...
# Clear Boost_INCLUDE_DIR if it did not change but other input affecting the location did.
# We will find a new one based on the new inputs.
...
if(NOT Boost_INCLUDE_DIR)
    ...
    elseif(_ENV_BOOST_ROOT)
        list(APPEND _boost_INCLUDE_SEARCH_DIRS ${_ENV_BOOST_ROOT}/include
             ${_ENV_BOOST_ROOT})
    endif()
    ...
    if(Boost_NO_SYSTEM_PATHS)
        ...
        list(APPEND _boost_INCLUDE_SEARCH_DIRS PATHS C:/boost/include C:/boost
             /sw/local/include)
    endif()

    # Try to find Boost by stepping backwards through the Boost versions we know about.
    # Build a list of path suffixes for each version.
    set(_boost_PATH_SUFFIXES)
    foreach(_boost_VER ${_boost_TEST_VERSIONS})
        # Add in a path suffix, based on the required version, ideally we could read this
        # from version.hpp, but for that to work we'd need to know the include dir already
        ...
        elseif(_boost_VER MATCHES "([0-9]+)\\.([0-9]+)")
            set(_boost_BOOSTIFIED_VERSION "${CMAKE_MATCH_1}_${CMAKE_MATCH_2}")
        endif()
...
# Set Boost_FOUND based only on header location and version. It will be updated below for
# component libraries.
if(Boost_INCLUDE_DIR)
    if(Boost_DEBUG)
        message(STATUS "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] "
    ...
    file(STRINGS "${Boost_INCLUDE_DIR}/boost/version.hpp" _boost_VERSION_HPP_CONTENTS
    ...
    foreach(v VERSION LIB_VERSION)
        if("${_boost_VERSION_HPP_CONTENTS}" MATCHES
           "#define BOOST_${v} ${_Boost_${v}_REGEX}")
    ...
    if(Boost_FIND_VERSION)
        ...
        elseif(Boost_FIND_VERSION_EXACT AND NOT "${_Boost_VERSION}" VERSION_EQUAL
                                            "${Boost_FIND_VERSION}")
            set(Boost_FOUND 0)
        ...
        if(NOT Boost_FOUND)
            ...
            if(Boost_FIND_VERSION_PATCH)
                string(APPEND Boost_ERROR_REASON ".${Boost_FIND_VERSION_PATCH}")
            endif()
...
if((GHSMULTI AND Boost_USE_STATIC_LIBS)
   OR (WIN32
...
if(Boost_COMPILER)
    ...
    # experience any issues please report them and use the Boost_COMPILER variable to work
    # around the problems.
...
# Key       Use this library when: s        linking statically to the C++ standard library
...
# n        using the STLport deprecated "native iostreams" feature removed from the
# documentation in 1.43.0 but still present in boost/config/auto_link.hpp
if(Boost_USE_STLPORT_DEPRECATED_NATIVE_IOSTREAMS)
    string(APPEND _boost_RELEASE_ABI_TAG "n")
    string(APPEND _boost_DEBUG_ABI_TAG "n")
endif()

# -x86     Architecture and address model tag First character is the architecture, then
# word-size, either 32 or 64 Only used in 'versioned' layout, added in Boost 1.66.0
if(DEFINED Boost_ARCHITECTURE)
    ...
    if(NOT "x${CMAKE_CXX_COMPILER_ARCHITECTURE_ID}" STREQUAL "x" AND NOT Boost_VERSION
                                                                     VERSION_LESS 106600)
        string(APPEND _boost_ARCHITECTURE_TAG "-")
        # This needs to be kept in-sync with the section of CMakePlatformId.h.in inside
...
foreach(c DEBUG RELEASE)
    ...
    if(Boost_LIBRARY_DIR_${c})
        set(_boost_LIBRARY_SEARCH_DIRS_${c} ${Boost_LIBRARY_DIR_${c}} NO_DEFAULT_PATH
        ...
        if(Boost_NO_SYSTEM_PATHS)
            ...
            foreach(ver ${_boost_TEST_VERSIONS})
                string(REPLACE "." "_" ver "${ver}")
                _boost_update_windows_library_search_dirs_with_prebuilt_paths(
                    _boost_LIBRARY_SEARCH_DIRS_${c} "C:/local/boost_${ver}")
            endforeach()
            _boost_update_windows_library_search_dirs_with_prebuilt_paths(
                _boost_LIBRARY_SEARCH_DIRS_${c} "C:/boost")
            list(APPEND _boost_LIBRARY_SEARCH_DIRS_${c} PATHS C:/boost/lib C:/boost
                 /sw/local/lib)
        endif()
    endif()
endforeach()
...
# The previous behavior of FindBoost when Boost_USE_STATIC_LIBS was enabled on WIN32 was
# to: 1. Search for static libs compiled against a SHARED C++ standard runtime library
# (use if found) 2. Search for static libs compiled against a STATIC C++ standard runtime
# library (use if found) We maintain this behavior since changing it could break people's
# builds. To disable the ambiguous behavior, the user need only set
# Boost_USE_STATIC_RUNTIME either ON or OFF.
...
# On versions < 1.35, remove the System library from the considered list since it wasn't
# added until 1.35.
...
# Additional components may be required via component dependencies. Add any missing
...
if(_Boost_CHANGE_LIBDIR_DEBUG
   OR _Boost_CHANGE_LIBDIR_RELEASE
   OR _Boost_CHANGE_LIBNAME)
...
foreach(COMPONENT ${Boost_FIND_COMPONENTS})
    ...
    if(${COMPONENT} STREQUAL "mpi"
       OR ${COMPONENT} STREQUAL "mpi_python"
       OR ${COMPONENT} STREQUAL "graph_parallel")
    ...
    # if Boost_LIBRARY_DIR_RELEASE is not defined, but Boost_LIBRARY_DIR_DEBUG is, look
    # there first for RELEASE libs
    ...
    # if Boost_LIBRARY_DIR_DEBUG is not defined, but Boost_LIBRARY_DIR_RELEASE is, look
    # there first for DEBUG libs
...
# The above setting of Boost_FOUND was based only on the header files. Update it for the
# requested component libraries.
if(Boost_FOUND)
    ...
    if(Boost_DEBUG)
        message(
            STATUS
                "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] Boost_FOUND = ${Boost_FOUND}"
            )
    endif()

    if(_Boost_MISSING_COMPONENTS)
        set(Boost_FOUND 0)
        # We were unable to find some libraries, so generate a sensible error message that
        # lists the libraries we were unable to find.
    ...
    if(NOT Boost_LIBRARY_DIRS AND NOT _boost_CHECKED_COMPONENT)
        # Compatibility Code for backwards compatibility with CMake 2.4's FindBoost
        # module.

        # Look for the boost library path. Note that the user may not have installed any
        # libraries so it is quite possible the Boost_LIBRARY_DIRS may not exist.
        ...
        if(_Boost_IMPORTED_TARGETS AND NOT TARGET Boost::${COMPONENT})
            ...
            if(Boost_${UPPERCOMPONENT}_FOUND)
                if(Boost_USE_STATIC_LIBS)
                    ...
                    # Even if Boost_USE_STATIC_LIBS is OFF, we might have static libraries
                    # as a result.
                    add_library(Boost::${COMPONENT} UNKNOWN IMPORTED)
                endif()
                if(Boost_INCLUDE_DIRS)
                    set_target_properties(
                        Boost::${COMPONENT} PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                                       "${Boost_INCLUDE_DIRS}")
                endif()
                ...
                if(_Boost_${UPPERCOMPONENT}_DEPENDENCIES)
                    ...
                    foreach(dep ${_Boost_${UPPERCOMPONENT}_DEPENDENCIES})
                        list(APPEND _Boost_${UPPERCOMPONENT}_TARGET_DEPENDENCIES
                             Boost::${dep})
                    endforeach()
...
if(Boost_FOUND)
    ...
    if(Boost_FIND_REQUIRED)
        ...
        if(NOT Boost_FIND_QUIETLY)
            # we opt not to automatically output Boost_ERROR_REASON here as it could be
            # quite lengthy and somewhat imposing in its requests Since Boost is not
            # always a required dependency we'll leave this up to the end-user.
...
# Record last used values of input variables so we can detect on the next run if the user
# changed them.
foreach(v ${_Boost_VARS_INC} ${_Boost_VARS_LIB} ${_Boost_VARS_DIR} ${_Boost_VARS_NAME})
    if(DEFINED ${v})
        set(_${v}_LAST
            "${${v}}"
